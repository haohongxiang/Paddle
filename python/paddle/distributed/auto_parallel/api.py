...
from paddle.io.dataloader.batch_sampler import DistributedBatchSampler
...
class DistModel:
    ...
    Args:
        ...
        loader(ShardDataLoader): The data loader used in dygraph mode,
...
class ShardDataloader:
    ...
    if has_dataset_splitted is True, just need to do reshard.

    Args:
        dataloader (paddle.io.DataLoader): The dataloader to be sharded.
        meshes (Union(list, tuple)): The mesh list of the dataloader. Identify which mesh the input is on.
        input_keys Union(list, tuple): The input keys of the outputs of dataloader if it is a dict.
            The input corresponding to input_keys[i] is on meshes[i]. if outputs is a list, no need to pass it,
            and the i'th input is on meshes[i].
        shard_dims (Union[list, tuple, str, int]): The mesh dimension to shard the dataloader.
            Users can specify the shard_dim of each mesh or specify a single shard_dim for all meshes.
        has_dataset_splitted (bool): Whether the dataset has been splitted.
    """

    def __init__(
        self,
        dataloader: paddle.io.DataLoader,
        meshes: List[dist.ProcessMesh],
        input_keys: List[str] = None,
        shard_dims: Union[list, tuple, str, int] = None,
        has_dataset_splitted: bool = False,
    ):
        # do some check
        if has_dataset_splitted is True and shard_dims is None:
            raise ValueError(
                "shard_dims must be set when has_dataset_splitted is True"
            )
        self._meshes = meshes
        self._all_inputs_in_one_mesh = len(self._meshes) == 1
        self._input_keys = input_keys
        self._shard_dims = self.process_shard_dims(shard_dims)
    ...
    def current_process_in_which_mesh(self, process_id):
    ...
    def _get_batch(self, batch_data):
        if isinstance(batch_data, (list, tuple)):
            ...
            dist_batch_data = []
            for ind in range(len(batch_data)):
                shard_dim = (
                    self._shard_dims[0]
                    if self._all_inputs_in_one_mesh
                    else self._shard_dims[ind]
                )
...
def shard_dataloader(
    ...
    """
    Convert the dataloader to a ShardDataloader. See ShardDataloader for details

    Args:
        dataloader (paddle.io.DataLoader): The dataloader to be sharded.
        meshes (Union(list, tuple)): The mesh list of the dataloader.
            Identify which mesh the input is on. if len(meshes) == 1,
            all the inputs are on the same mesh.
        input_keys List[str]: The input keys of the dataloader outputs if it is a dict.
            The input corresponding to input_keys[i] is on meshes[i]. if outputs is a list,
            there is no need to pass it, and the i'th input is on meshes[i].
        shard_dims (Union[list, tuple, str, int]): The mesh dimension to shard the dataloader.
            if shard_dims is None, The data loader will not be split, i.e. mp.
            Users can specify the shard_dim of each mesh or specify a single shard_dim for all meshes.
        has_dataset_splitted (bool): Whether the dataset has been splitted.
    Returns:
    ...
    return ShardDataloader(
        dataloader, meshes, input_keys, shard_dims, has_dataset_splitted
    )
